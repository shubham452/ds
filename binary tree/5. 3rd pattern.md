Perfect.
Now we move to the **MOST IMPORTANT + MOST CONFUSING** tree pattern — and we’ll do it **slowly and correctly**.

---

# 🌳 PATTERN 3 — PATH PROBLEMS (MASTER PATTERN)

> **If you master this pattern, you unlock the hardest tree questions.**

Most people fail here because they don’t understand **what a path really is**.

---

## 🧠 FIRST: WHAT IS A “PATH” IN A TREE?

A **path** is:

* A sequence of connected nodes
* **No branching**
* Each node has **at most one parent and one child in the path**

❌ This is NOT a path:

```
   10
  /  \
 5   20
```

✔ This IS a path:

```
5 → 10 → 20
```

---

## 🧠 CRITICAL RULE (WRITE THIS DOWN)

> **A path can fork ONLY when updating the answer, NEVER when returning.**

This rule will save you from 90% of mistakes.

---

## 🔍 HOW TO IDENTIFY PATH PROBLEMS

Look for these words:

* “Path”
* “Sum”
* “Maximum”
* “From root to leaf”
* “Any node to any node”
* “Lowest Common Ancestor”

If you see these → **Pattern 3**

---

## 🧩 TWO TYPES OF PATH PROBLEMS (VERY IMPORTANT)

---

## 🔹 TYPE A — ROOT → LEAF PATHS

### Examples

* Path Sum
* Path Sum II
* Binary Tree Paths
* Sum Root to Leaf Numbers

### Concept

> Path always starts at root
> No global answer
> Pass info **downward**

---

### 🧱 TEMPLATE (TOP-DOWN DFS)

```cpp
void dfs(TreeNode* root, int currSum) {
    if (!root) return;

    currSum += root->val;

    if (!root->left && !root->right) {
        // leaf reached → check/update answer
    }

    dfs(root->left, currSum);
    dfs(root->right, currSum);
}
```

### Key Idea

* No return value
* You **carry the path**

---

## 🔹 TYPE B — ANY NODE → ANY NODE (HARDEST)

### Examples

* Binary Tree Maximum Path Sum
* Path Sum III (variation)

### Concept

> Path can start and end anywhere
> DFS returns **best downward path**
> Global answer handles fork

![Image](https://media.geeksforgeeks.org/wp-content/cdn-uploads/tree.png)

![Image](https://afteracademy.com/images/path-sum-in-binary-tree-fb1857ace44dccc1.png)

---

## 🧠 THIS IS THE CORE LOGIC (MEMORIZE)

### What DFS returns

> **Maximum downward path starting at this node**

### What GLOBAL stores

> **Maximum path seen so far (may fork)**

---

## 🧱 UNIVERSAL TEMPLATE (TYPE B)

```cpp
int ans = INT_MIN;

int dfs(TreeNode* root) {
    if (!root) return 0;

    int left = max(0, dfs(root->left));
    int right = max(0, dfs(root->right));

    // 1️⃣ Update global answer (can fork)
    ans = max(ans, root->val + left + right);

    // 2️⃣ Return ONE path upward (no fork)
    return root->val + max(left, right);
}
```

🔥 This is one of the **most reused templates in interviews**.

---

## 🧠 WHY `max(0, …)` ?

Because:

* Negative paths only reduce sum
* You are allowed to **stop the path**

---

## 🧠 LIVE INTERVIEW EXPLANATION (USE THIS)

Say this confidently:

> “This is an any-node to any-node path problem.
> My DFS returns the maximum downward path from the current node.
> At each node, I update a global maximum using left + right + node.
> When returning to the parent, I return only one side because paths cannot fork upward.”

This sounds **senior-level**.

---

## 🧪 STEP-BY-STEP DRY RUN (IMPORTANT)

Tree:

```
      10
     /  \
    2    10
         / \
        -5  20
```

At node `20` → return 20
At node `-5` → return 0
At node `10` (right):

* left = 0, right = 20
* ans = max(ans, 10 + 0 + 20 = 30)
* return 30

At root `10`:

* left = 2, right = 30
* ans = max(ans, 42)

✔ Final answer = **42**

---

## ⚠️ COMMON MISTAKES (DON’T DO THESE)

❌ Returning `left + right`
❌ Forgetting global variable
❌ Allowing fork in return
❌ Mixing root→leaf logic with any→any

---

## 🧭 QUICK DECISION TABLE

| Problem           | Pattern     |
| ----------------- | ----------- |
| Path Sum I        | Root → Leaf |
| Path Sum II       | Root → Leaf |
| Binary Tree Paths | Root → Leaf |
| Max Path Sum      | Any → Any   |
| Path Sum III      | Any → Any   |

---

## 📌 ONE-LINE MEMORY HOOK

> **Return ONE path, update TWO paths**

If you remember this → you’ll never mess up.

---

## 🧪 YOUR TURN (VERY IMPORTANT)

Without coding, answer this:

Tree:

```
      5
     / \
    4   8
       / \
      11  13
```

Questions:

1. What does `dfs(11)` return?
2. What does `dfs(8)` return?
3. What is the maximum path sum?

Take your time.
This will cement Pattern 3 permanently.


Excellent question — this confusion is **normal** and clearing it properly is what separates **average** from **strong** tree solvers.

Let me explain this **slowly, intuitively, and visually (in words)**.

---

# 🔑 Statement to Understand

> **“A path can fork ONLY when updating the answer, NEVER when returning.”**

This single sentence is the **core rule of all hard path problems**.

---

## 🧠 First: What does “fork” mean?

A **fork** means taking **both left and right children** at the same node.

Example:

```
    10
   /  \
  5    20
```

The forked path is:

```
5 → 10 → 20
```

Here, node `10` connects **two directions**.

---

## 🧠 What does “returning” mean?

“Returning” means:

* What your `dfs(node)` gives **back to its parent**
* This value will be used by the parent to build **its own path**

So ask yourself:

> “What information can a parent use?”

---

## 🚫 WHY A PATH CANNOT FORK WHEN RETURNING

Imagine this situation:

```
      P
      |
      10
     /  \
    5    20
```

If `dfs(10)` returned **both sides (5 + 10 + 20)** to parent `P`,
then parent would try to do:

```
P → 10 → 5
        → 20
```

❌ That is **NOT a path**
❌ That is a **tree / graph**, not a line

👉 **Paths must be linear**.

So when returning:

> **You MUST choose only one direction (left OR right)**

---

## ✅ WHAT IS ALLOWED WHEN RETURNING

From node `10`, you can return:

* `10 → 5`
* OR `10 → 20`

But **never both**.

That’s why we do:

```cpp
return root->val + max(left, right);
```

---

## ✅ WHY FORKING IS ALLOWED WHEN UPDATING THE ANSWER

Now this part is crucial.

When you do:

```cpp
ans = max(ans, root->val + left + right);
```

You are saying:

> “The best path **passing through THIS node** might use both sides.”

This is fine because:

* This path **starts and ends here**
* It is **complete**
* It will **not be extended upward**

So forking is allowed **only here**.

---

## 🧠 THINK OF IT LIKE THIS (VERY IMPORTANT)

### Returning value:

> “Can my parent extend this path?”

So it must be:

* Single direction
* Linear

### Updating global answer:

> “Is this the best path that ends here?”

So it can:

* Use both children
* Fork once

---

## 🧪 STEP-BY-STEP EXAMPLE (CRYSTAL CLEAR)

Tree:

```
      10
     /  \
    5    20
```

### At node `10`:

* `left = 5`
* `right = 20`

#### 1️⃣ Update global answer (fork allowed)

```
5 → 10 → 20  = 35
```

✔ Valid complete path

#### 2️⃣ Return to parent (fork NOT allowed)

You must choose:

```
10 → 20  (better)
```

So return:

```
30
```

---

## ❌ WHAT GOES WRONG IF YOU FORK WHEN RETURNING

If you returned `5 + 10 + 20 = 35`,
parent would try to attach something like:

```
X → 35
```

Which is impossible in a tree path.

---

## 🧠 ONE-SENTENCE MEMORY RULE (WRITE THIS DOWN)

> **“Fork once to compute the answer, never to extend the path upward.”**

or even simpler:

> **“Return one branch, score both branches.”**

---

## 🧩 WHY THIS RULE SOLVES ALL PATH PROBLEMS

Because every path problem boils down to:

* **Extension upward** → must be linear
* **Final answer** → may combine two sides

Once this clicks, problems like:

* Max Path Sum
* Path Sum III
* Diameter
  become **mechanical**.

---

## ✅ QUICK CHECK (ANSWER YES / NO)

1. Can a returned path have two children?
2. Can the global answer include left + right + root?
3. Is returning `root + max(left, right)` mandatory?

Reply with **YES/NO answers**.
After that, we’ll do **one live dry run together** to lock this forever.


This is **exactly the right moment to correct and solidify** the concept.
You understood **part of it**, but one answer is **intentionally wrong**, and fixing it will make everything click.

Let’s go **line by line**, like a real classroom.

---

## ❓ Q1: Can a returned path have two children?

**Your answer:** ❌ no
**Correct answer:** ✅ **NO (correct)**

### Why?

A returned path goes **upward to the parent**.
A parent can only extend **one straight line**, not a fork.

So this is **correctly understood** 👍

---

## ❓ Q2: Can the global answer include `left + right + root`?

**Your answer:** ❌ no
**Correct answer:** ✅ **YES (this is the key correction)**

### This is the MOST IMPORTANT FIX 🔥

The **global answer is ALLOWED to fork**.

Why?

Because the global answer represents:

> “The best COMPLETE path that ends at this node.”

That path **does not go upward** anymore.
It starts and ends inside the subtree.

### Example

```
    10
   /  \
  5    20
```

The path:

```
5 → 10 → 20
```

✔ This is a **valid path**
✔ It forks **once**
✔ It is a **final answer**, not extended upward

So:

```cpp
ans = max(ans, root->val + left + right);  // ✅ VALID
```

If this were not allowed, **Max Path Sum and Diameter would be impossible**.

👉 **So the correct answer is YES**.

---

## ❓ Q3: Is returning `root + max(left, right)` mandatory?

**Your answer:** ✅ yes
**Correct answer:** ✅ **YES (correct)**

### Why?

Because when returning:

* The path must stay **linear**
* Parent can extend only **one side**

So you MUST choose the better side:

```cpp
return root->val + max(left, right);
```

---

## ✅ CORRECTED FINAL ANSWERS (LOCK THESE)

| Question                                           | Correct |
| -------------------------------------------------- | ------- |
| Can a returned path have two children?             | ❌ NO    |
| Can the global answer include left + right + root? | ✅ YES   |
| Is returning root + max(left, right) mandatory?    | ✅ YES   |

---

## 🧠 THE FINAL, PERFECT RULE (WRITE THIS DOWN)

> **Fork ONCE to UPDATE the answer.
> NEVER fork when RETURNING upward.**

Or even shorter:

> **Return one branch, score both branches.**

This single rule unlocks:

* Max Path Sum
* Diameter
* Path Sum III
* Any-node path problems

---

## 🔒 LET’S LOCK IT WITH ONE LAST MICRO-DRY-RUN

Tree:

```
      1
     / \
    2   3
```

* At node `1`:

  * left = 2
  * right = 3

### Global answer:

```
2 + 1 + 3 = 6  ✅ allowed
```

### Returned value:

```
1 + max(2, 3) = 4  ✅ mandatory
```

No contradiction.
Different purposes.

---

## 🎯 YOU ARE NOW READY

This confusion is **the hardest conceptual barrier** in trees.
You just crossed it.

Next, we will:
👉 Solve **Max Path Sum LIVE, step by step**
👉 Then move to **Pattern 4 (Two-tree comparison)**

Reply with:

* **“Live max path sum”**
  or
* **“Next pattern”**

You’re doing very well. Keep going 👊


A path can fork ONLY when updating the answer, NEVER when returning.


Great — this is **the key conceptual question of Pattern 3**.
If this clicks, **all hard path problems click**.

I’ll explain **exactly** what “fork allowed here, not here” means, with **no jargon**.

---

# 🔑 First: what does “fork” mean?

A **fork** means:

> Using **both left and right child paths** at the same node.

Visually:

```
    X
   / \
  L   R
```

Using **L + X + R** is a **fork**.

---

# 🔍 Now look at the two places in the code

```cpp
ans = max(ans, root->val + left + right);   // fork allowed here
return root->val + max(left, right);        // fork NOT allowed here
```

They serve **two completely different purposes**.

---

# 🟢 1️⃣ “Fork allowed here” → updating `ans`

```cpp
ans = max(ans, root->val + left + right);
```

### What is `ans`?

* `ans` stores the **final answer**
* It represents a **complete path**
* That path **starts and ends here**
* It will **NOT be extended upward**

### So what path are we considering?

```
(left subtree) → root → (right subtree)
```

Example:

```
    10
   /  \
  5    20
```

Path:

```
5 → 10 → 20
```

✔ This is a **valid path**
✔ Paths are allowed to fork **once**
✔ This path is **finished**

👉 That’s why **forking is allowed when updating `ans`**.

---

# 🔴 2️⃣ “Fork NOT allowed here” → returning to parent

```cpp
return root->val + max(left, right);
```

### What does this return value mean?

It means:

> “If my parent wants to continue a path through me,
> what is the **best single direction** it can take?”

A path going **upward** must be **linear**.

---

## ❌ Why can’t we fork when returning?

Suppose you returned:

```cpp
root->val + left + right
```

Then the parent would try to extend **this**:

```
      P
      |
      X
     / \
    L   R
```

That would create:

```
P → X → L
     ↘
      R
```

❌ That is **not a path**
❌ That is a **branching structure**

A path must be a **straight line**.

So when returning:

* You **must choose only one side**
* That’s why we do `max(left, right)`

---

# 🧠 The core rule (memorize this)

> **A path may fork only when it ends, never when it continues.**

---

# 🧩 Think of it like a road

* Updating `ans` = road **ends here**

  * You can come from left and go to right
* Returning value = road **continues upward**

  * A road can only go in **one direction**

---

# 🧪 Small concrete example

Tree:

```
    1
   / \
  2   3
```

### At node `1`:

* `left = 2`
* `right = 3`

#### ✅ Global answer (fork allowed)

```
2 → 1 → 3 = 6
```

#### ✅ Returned value (no fork)

```
1 → 3 = 4
```

We **score both**, but **return one**.

---

# 🔒 One-line memory hook (VERY IMPORTANT)

> **Return ONE branch, score BOTH branches.**

Or:

> **Fork once to score, never to return.**

---

# 🧠 Final clarity check

* Fork allowed when?
  👉 **When computing the final answer at a node**

* Fork not allowed when?
  👉 **When returning a path to the parent**

That’s it. Nothing more mystical than that.

---

If you want next, we can:

* Dry-run this code on a tricky tree **together**
* Compare Pattern 2 vs Pattern 3 side-by-side
* Solve **Path Sum III** (which mixes ideas)

Just tell me 👊

